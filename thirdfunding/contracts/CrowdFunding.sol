...
contract CrowdFunding {
    ...
    function getCampaignsById(uint256 _id)
        public
        view
    ...
    function getCampaignsByCategory(string memory _category)
        ...
        returns (Campaign[] memory)
    {
        Campaign[] memory allCampaignsByCategory = new Campaign[](
            numberOfCampaigns
        );
        ...
        for (uint256 i = 0; i < numberOfCampaigns; i++) {
            if (
                keccak256(abi.encodePacked(campaigns[i].category)) ==
                keccak256(abi.encodePacked(_category))
            ) {
                Campaign storage item = campaigns[i];
                allCampaignsByCategory[index] = item;
                index++;
            }
        }
